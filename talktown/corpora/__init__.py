...
class Names:
    ...
    miscellaneous_feminine_forenames = tuple(
        name[:-1] for name in
        open(Path(__file__).parent.absolute() / 'feminine_names.txt', 'r')
    )
    english_surnames = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'english_surnames.txt', 'r')
    )
    french_surnames = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'french_surnames.txt', 'r')
    )
    german_surnames = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'german_surnames.txt', 'r')
    )
    irish_surnames = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'irish_surnames.txt', 'r')
    )
    scandinavian_surnames = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'scandinavian_surnames.txt', 'r')
    )
    all_surnames = (
    ...
    place_names = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'US_settlement_names.txt', 'r')
    )
    restaurant_names = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'restaurant_names.txt', 'r')
    )
    bar_names = tuple(
        name.strip('\n') for name in
        open(Path(__file__).parent.absolute() / 'bar_names.txt', 'r')
    )
...
class GravestoneDetails:
    ...
    headers = tuple(
        header.strip('\n') for header in
        open(Path(__file__).parent.absolute() / 'gravestone_headers.txt', 'r')
    )
    epitaphs = tuple(
        epitaph.strip('\n') for epitaph in
        open(Path(__file__).parent.absolute() / 'gravestone_epitaphs.txt', 'r').read().split('\n\n')
    )
